...
and easily. As of April 2020, nearly 1 million SRA datasets have been
processed.
...
For more information wbout working with SummarizedExperiment refer to the
...
The function getDEE2 is provided for completeness but is no longer
recommended. It gives DEE2 data in the form of a list object with slots
for gene counts, transcript counts, gene length, transcript length, quality
control data, sample metadata summary, sample metadata (full) and any absent
datasets.

```{r, legacy}
x<-getDEE2_legacy("celegans",SRRvec)
names(x)
head(x$GeneCounts)
head(x$TxCounts)
head(x$QcMx)
...
runs at the [link](http://dee2.io/huge). We have created some functions to
enable programmatic access to these data as well.
...
In this first example we are looking to obtain a dataset with SRA project 
...
x <- getDEE2_bundle("drerio", "GSE106677",
    col="GSE_accession",counts="Tx2Gene")
    assays(x)$counts[1:6,1:4]
```
